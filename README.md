...
-**Extract**: The `api_client.py` uses a "Golden Ticket" refresh token to generate a fresh access token for every run. It then fetches the latest activities from the Strava API and stores them in a local SQLite database (`strava.db`) to prevent duplicates.
-**Transform**: Using `pandas`, the `transform.py` script converts raw SI units (meters, seconds) into athlete-friendly metrics like miles and feet. It calculates weekly totals and formats average paces into "MM:SS" strings.
...
| **`init_db.py`**         | Utility script to initialize the SQLite schema for local development.                             |
| **`.env.example`**       | A template for required Strava API credentials and refresh tokens.                                |
...
Create a `.env` file based on the provided example and fill in your Strava credentials:
